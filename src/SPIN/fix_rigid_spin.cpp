...
FixRigidSpin::FixRigidSpin(LAMMPS *lmp, int narg, char **arg) :
...
void FixRigidSpin::setup_pre_neighbor()
{
  pre_neighbor();
}
...
void FixRigidSpin::setup(int vflag)
{
  int i,n,ibody;
...
void FixRigidSpin::initial_integrate(int vflag)
{
  double dtfm;
...
void FixRigidSpin::apply_langevin_thermostat()
{
  if (me == 0) {
...
void FixRigidSpin::compute_forces_and_torques()
{
  int i,ibody;
...
void FixRigidSpin::post_force(int /*vflag*/)
{
...
void FixRigidSpin::final_integrate()
{
  int ibody;
  double dtfm;
...
void FixRigidSpin::initial_integrate_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixRigidSpin::final_integrate_respa(int ilevel, int /*iloop*/)
{
  dtf = 0.5 * step_respa[ilevel] * force->ftm2v;
  final_integrate();
}
...
void FixRigidSpin::pre_neighbor()
{
  for (int ibody = 0; ibody < nbody; ibody++)
...
int FixRigidSpin::dof(int tgroup)
{
...
void FixRigidSpin::deform(int flag)
{
  if (flag == 0)
    for (int ibody = 0; ibody < nbody; ibody++)
...
void FixRigidSpin::setup_bodies_static()
{
  int i,ibody;
...
void FixRigidSpin::setup_bodies_dynamic()
{
  int i,ibody;
...
void FixRigidSpin::readfile(int which, double *vec,
...
void FixRigidSpin::write_restart_file(char *file)
{
  if (me) return;
...
double FixRigidSpin::memory_usage()
{
  int nmax = atom->nmax;
...
void FixRigidSpin::grow_arrays(int nmax)
{
...
void FixRigidSpin::copy_arrays(int i, int j, int /*delflag*/)
{
  body[j] = body[i];
...
void FixRigidSpin::set_arrays(int i)
{
  body[i] = -1;
  xcmimage[i] = 0;
  displace[i][0] = 0.0;
...
int FixRigidSpin::pack_exchange(int i, double *buf)
{
  buf[0] = ubuf(body[i]).d;
...
int FixRigidSpin::unpack_exchange(int nlocal, double *buf)
{
...
void FixRigidSpin::zero_momentum()
{
  for (int ibody = 0; ibody < nbody; ibody++)
    vcm[ibody][0] = vcm[ibody][1] = vcm[ibody][2] = 0.0;
...
void FixRigidSpin::zero_rotation()
{
  for (int ibody = 0; ibody < nbody; ibody++) {
    angmom[ibody][0] = angmom[ibody][1] = angmom[ibody][2] = 0.0;
...
int FixRigidSpin::modify_param(int narg, char **arg)
{
...
double FixRigidSpin::compute_scalar()
{
...
void *FixRigidSpin::extract(const char *str, int &dim)
{
...
double FixRigidSpin::extract_ke()
{
  double ke = 0.0;
  for (int i = 0; i < nbody; i++)
    ke += masstotal[i] *
...
double FixRigidSpin::extract_erotational()
{
  double wbody[3],rot[3][3];
...
double FixRigidSpin::compute_array(int i, int j)
{
